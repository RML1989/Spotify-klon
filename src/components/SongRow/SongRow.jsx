                ...
                <Grid item sx={{flex: 1, display: 'flex', alignItems: 'center', gap: 2}}>
                    ...
                    <Box>
                        <Typography sx={{fontSize: 16, color: 'text.primary'}}>
                            {loading ? <Skeleton variant="text" sx={{ width: 130, height: 24 }}/> : title}
                            </Typography>
                        <Typography sx={{fontSize: 12, color: 'text.secondary'}}>
                            {loading ? <Skeleton variant="text" sx={{ width: 50, height: 18 }}/> : artist}
                        </Typography>
                    </Box>
                </Grid>
                <Grid xs={3} item sx={{display: {xs: 'none', md: 'flex'}, alignItems: 'center'}}>
                    {loading ? <Skeleton variant="text" sx={{ width: 50, height: 24 }}/> : album}
                </Grid>
                <Grid xs={3} item sx={{display: 'flex', alignItems: 'center', justifyContent: 'flex-end'}}>
                    {loading ? <Skeleton variant="text" sx={{ width: 50, height: 24 }}/> : formatTime(duration)}
                </Grid>
